...
package com.mts2792.launcher.provider;
...
import com.mts2792.launcher.LauncherProvider.DatabaseHelper;
...
public class RestoreDbTask {
    ...
    private void sanitizeDB(DatabaseHelper helper, SQLiteDatabase db) throws Exception {
        ...
        int itemsDeleted = db.delete(
                LauncherSettings.Favorites.TABLE_NAME, "profileId != ?", new String[]{Long.toString(oldProfileId)});
        if (itemsDeleted > 0) {
        ...
        values.put(LauncherSettings.Favorites.RESTORED, ShortcutInfo.FLAG_RESTORED_ICON
                | (keepAllIcons ? ShortcutInfo.FLAG_RESTORE_STARTED : 0));
        db.update(LauncherSettings.Favorites.TABLE_NAME, values, null, null);
        ...
        db.update(LauncherSettings.Favorites.TABLE_NAME, values, "itemType = ?",
                new String[]{Integer.toString(LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET)});
    ...
    protected void migrateProfileId(SQLiteDatabase db, long newProfileId) {
        ...
        ContentValues values = new ContentValues();
        values.put(LauncherSettings.Favorites.PROFILE_ID, newProfileId);
        db.update(LauncherSettings.Favorites.TABLE_NAME, values, null, null);
    ...
    protected long getDefaultProfileId(SQLiteDatabase db) throws Exception {
        try (Cursor c = db.rawQuery("PRAGMA table_info (favorites)", null)){
            ...
            while (c.moveToNext()) {
                if (LauncherSettings.Favorites.PROFILE_ID.equals(c.getString(nameIndex))) {
