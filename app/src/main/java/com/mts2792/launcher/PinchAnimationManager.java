...
public class PinchAnimationManager {
    ...
    public void animateThreshold(float threshold, Workspace.State startState,
        ...
        if (threshold == PinchThresholdManager.THRESHOLD_ONE) {
            if (startState == Workspace.State.OVERVIEW) {
                animateOverviewPanelButtons(goingTowards == Workspace.State.OVERVIEW);
            } else if (startState == Workspace.State.NORMAL) {
                animateHotseatAndQsb(goingTowards == Workspace.State.NORMAL);
            }
        } else if (threshold == PinchThresholdManager.THRESHOLD_TWO) {
            if (startState == Workspace.State.OVERVIEW) {
                animateHotseatAndQsb(goingTowards == Workspace.State.NORMAL);
                animateScrim(goingTowards == Workspace.State.OVERVIEW);
            } else if (startState == Workspace.State.NORMAL) {
                animateOverviewPanelButtons(goingTowards == Workspace.State.OVERVIEW);
                animateScrim(goingTowards == Workspace.State.OVERVIEW);
            }
            ...
            if (startState == Workspace.State.OVERVIEW && goingTowards == Workspace.State.NORMAL) {
                mLauncher.showWorkspace(true);
                mWorkspace.snapToPage(mWorkspace.getCurrentPage());
            } else if (startState == Workspace.State.NORMAL && goingTowards == Workspace.State.OVERVIEW) {
